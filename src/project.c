...
void projectObject(Camera* cam,Rasterizer* r,vec3 lightSource,Object* obj){
  ...
  mat4 objToCamera;
  mat4 objectToProject;
  multmat4mat4Ptr(&objToCamera,toCamera,objToWorld);
  float xRange = fabs(objToCamera.m[0][0])+fabs(objToCamera.m[0][1])+fabs(objToCamera.m[0][2]);
  float yRange = fabs(objToCamera.m[1][0])+fabs(objToCamera.m[1][1])+fabs(objToCamera.m[1][2]);
  float zRange = fabs(objToCamera.m[2][0])+fabs(objToCamera.m[2][1])+fabs(objToCamera.m[2][2]);
  float maxX = xRange + objToCamera.m[0][3];
  float minX = -xRange + objToCamera.m[0][3];
  float maxY = yRange + objToCamera.m[1][3];
  float minY = -yRange + objToCamera.m[1][3];
  float maxZ = zRange + objToCamera.m[2][3];
  float minZ = -zRange + objToCamera.m[2][3];
  if(maxZ < cam->frustumNear){
    IMEJ_DEBuG_LOG("NEAR\n");
    return;
  }
  if(minZ > cam->frustumFar){
    IMEJ_DEBuG_LOG("FAR\n");
    return;
  }
  float d = vec3DotProduct((vec3){maxX,0,maxZ},cam->frustumLeftNormal); 
  if(d > 0){
    IMEJ_DEBuG_LOG("LEFT\n");
    return; // object is left of the view frustum
  }
  d = vec3DotProduct((vec3){minX,0,maxZ},cam->frustumRightNormal); 
  if(d>0){
    IMEJ_DEBuG_LOG("RIGHT\n");
    return; // object is right of view frustum
  }
  d = vec3DotProduct((vec3){0,minY,maxZ},cam->frustumTopNormal); 
  if(d>0){
    IMEJ_DEBuG_LOG("ABOVE\n");
    return; // object is above the frustum
  }
  d = vec3DotProduct((vec3){0,maxY,maxZ},cam->frustumBaseNormal); 
  if(d>0){
    IMEJ_DEBuG_LOG("BELOW\n");
    return; // object is below the frustum
  }
  IMEJ_DEBuG_LOG("PAINTED\n");
  multmat4mat4Ptr(&objectToProject,&(cam->perspectiveMat),&objToCamera);
  Face f;
  ...
  for(int i=0;i<obj->faceCount;i++){
    ...
    vec3 normCameraSpace = paneNormal(p3,p2,p1);

    p1 = multMat4Vec3(objectToProject,p1);
    p2 = multMat4Vec3(objectToProject,p2);
    p3 = multMat4Vec3(objectToProject,p3);
    norm = paneNormal(p3,p2,p1);
    if(norm.z > 0){
      continue;
    }

    normCameraSpace = multMat4Vec3Normal(objToCamera,normCameraSpace);
    normCameraSpace = vec3Normalised(normCameraSpace);
    percent = vec3DotProduct(lightSource,normCameraSpace);
    if(percent > 0){
      newCol = scaleColour(objectColour,percent );
    }else{
      newCol = BLACKCOL;
    }
    if(
...
// }
//
void projectObjectOld(Camera* cam,Rasterizer* r,vec3 lightSource,Object* obj){
  static const float VERTDIV = 1.0f/VERTMAX;
  mat4* objToWorld = getObjectTransformationMatrix(obj);
  mat4* toCamera = getWorldToCameraMatrix(cam);
  mat4 objToCamera;
